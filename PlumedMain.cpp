...
// ***CHANGED***
// added GETCENTRE, GETCURRENTANG, SETAT (works only for RESTRAINT bias)
...
/// ***CHANGED***
void PlumedMain::setAt(void* new_at_val){
  double potato;
  atoms.MD2double(new_at_val, potato);
 
// calculate the active actions in order (assuming *backward* dependence)
   for(ActionSet::iterator p=actionSet.begin();p!=actionSet.end();++p){
      // set at value if action is active
      if((*p)->isActive()){
        // are we the correct action??
        if ((*p)->get_at(0) != -125256363) {
          (*p)->set_at(potato, 0);
          log << "set the potato " << potato << "\n";
        }
      }
